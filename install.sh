#!/usr/bin/env sh
# BladeCNet installer — Linux / macOS
# Usage: curl -fsSL https://raw.githubusercontent.com/FadingBlade/BladeCNet/main/install.sh | sh
...
echo ""
printf "${BOLD}${CYN}BladeCNet installer${RST} — terminal network dashboard\n"
echo ""
            ...
            case "$OS" in
                ...
                linux)
                    if command -v apt-get >/dev/null 2>&1; then
                        ...
                        fail "Cannot auto-install Python. Please install from https://python.org"
                    fi ;;
                *)
...
"$PYTHON" -m pip install psutil --quiet --break-system-packages 2>/dev/null \
    ...
    || warn "psutil install failed — BladeCNet will try to install it on first run."
ok "psutil ready"
...
say "Downloading BladeCNet..."
if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$REPO/BladeCNet.py" -o "$INSTALL_DIR/BladeCNet.py"
elif command -v wget >/dev/null 2>&1; then
    wget -q "$REPO/BladeCNet.py" -O "$INSTALL_DIR/BladeCNet.py"
else
    fail "curl or wget is required."
fi
chmod +x "$INSTALL_DIR/BladeCNet.py"
ok "Downloaded BladeCNet.py"

# ── Create cnet wrapper ───────────────────────────────────────────────────────

cat > "$INSTALL_DIR/cnet" << WRAPPER
#!/usr/bin/env sh
exec "$PYTHON" "$INSTALL_DIR/BladeCNet.py" "\$@"
WRAPPER
chmod +x "$INSTALL_DIR/cnet"
ok "Created cnet command"
...
echo ""
ok "${BOLD}BladeCNet installed!${RST}"
echo ""
printf "  ${CYN}cnet${RST}                   full overview\n"
printf "  ${CYN}cnet ip${RST}                IP + ISP info\n"
printf "  ${CYN}cnet devices${RST}           scan LAN devices\n"
printf "  ${CYN}cnet watch${RST}             live dashboard\n"
printf "  ${CYN}cnet help${RST}              all commands\n"
echo ""
warn "Note: cnet devices and cnet ports may need sudo for full results."
echo ""
